                    ...
                    this.particles.forEach(function (t, r) {
                        if (0 !== r) {
                            ...
                            var m = t.position
                                ...
                                .normalize();
                            (t.position = i.particles[r - 1].position.add(
                                m.multiplyByScalar(t.radius),
                            )),
                                Math.abs(t.position.x) < f.movementThreshold &&
                                (t.position.x = 0),
                                (t.velocity =
                                    0 !== a
                                        ? t.position
                                            ...
                                            .divideByScalar(a)
                                            .multiplyByScalar(t.mobility)
                                        : p.zero),
                                (t.force = p.zero),
                                (t.lastGravity = l);
                        } else t.position = new p(s.x, s.y);
                    });
                }),
    ...
    class a {
        ...
        onUpdate(e) {
            ...
            if (this.inDrag) {
                ...
                this.pointerX = lerp(this.pointerX, 0, damping);
                this.pointerY = lerp(this.pointerY, 0, damping);

                this.addParameterValueById("ParamAngleX", 5 * this.pointerX);
                this.addParameterValueById("ParamAngleY", 5 * -this.pointerY);
                this.addParameterValueById("ParamAngleZ", 5 * -this.pointerX);

                const eyeMovementX = lerp(this.pointerX, 0, damping);
                const eyeMovementY = lerp(this.pointerY, 0, damping);

                const maxEyeMovement = 1;
                const clampedEyeX = Math.max(-maxEyeMovement, Math.min(maxEyeMovement, eyeMovementX));
                const clampedEyeY = Math.max(-maxEyeMovement, Math.min(maxEyeMovement, eyeMovementY));

                this.addParameterValueById("ParamEyeBallX", clampedEyeX);
                this.addParameterValueById("ParamEyeBallY", clampedEyeY);
            }
